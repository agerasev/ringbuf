...
fn try_push() {
    ...
    assert_eq!(indices(prod.base_ref()), (0, 0));

    assert_eq!(prod.try_push(123), Ok(()));
    assert_eq!(indices(prod.base_ref()), (0, 1));

    assert_eq!(prod.try_push(234), Ok(()));
    assert_eq!(indices(prod.base_ref()), (0, 2));
...
fn pop_empty() {
    ...
    assert_eq!(indices(cons.base_ref()), (0, 0));
...
fn push_pop_one() {
    ...
    for (i, v) in values.iter().enumerate() {
        ...
        assert_eq!(indices(cons.base_ref()), (i % MOD, (i + 1) % MOD));

        assert_eq!(cons.try_pop().unwrap(), *v);
        assert_eq!(indices(cons.base_ref()), ((i + 1) % MOD, (i + 1) % MOD));
...
fn push_pop_all() {
    ...
    for (i, v) in values.iter().enumerate() {
        ...
        assert_eq!(indices(cons.base_ref()), (CAP * i % MOD, (CAP * i + 1) % MOD));

        assert_eq!(prod.try_push(v.1), Ok(()));
        assert_eq!(indices(cons.base_ref()), (CAP * i % MOD, (CAP * i + 2) % MOD));

        assert_eq!(prod.try_push(v.2).unwrap_err(), v.2);
        assert_eq!(indices(cons.base_ref()), (CAP * i % MOD, (CAP * i + 2) % MOD));

        assert_eq!(cons.try_pop().unwrap(), v.0);
        assert_eq!(indices(cons.base_ref()), ((CAP * i + 1) % MOD, (CAP * i + 2) % MOD));

        assert_eq!(cons.try_pop().unwrap(), v.1);
        assert_eq!(indices(cons.base_ref()), ((CAP * i + 2) % MOD, (CAP * i + 2) % MOD));
