...
impl<T, const N: usize> Default for AtomicRingBuffer<T, [MaybeUninit<T>; N]> {
    fn default() -> Self {
...
#[cfg(feature = "alloc")]
impl<T> AtomicRingBuffer<T, Vec<MaybeUninit<T>>> {
...
pub type StaticRingBuffer<T, const N: usize> = AtomicRingBuffer<T, [MaybeUninit<T>; N]>;

/// Heap-allocated ring buffer.
#[cfg(feature = "alloc")]
pub type HeapRingBuffer<T> = AtomicRingBuffer<T, Vec<MaybeUninit<T>>>;
