...
pub struct GlobalProducer<T, C, R>
where
    C: Container<T>,
    R: RingBufferRef<T, C>,
{
...
impl<T, C, R> GlobalProducer<T, C, R>
where
    C: Container<T>,
    R: RingBufferRef<T, C>,
{
    ...
    pub fn acquire(&mut self) -> LocalProducer<'_, T, C> {
        LocalProducer::new(&self.ring_buffer.storage, unsafe {
    ...
    pub fn transfer_from<Cs, Rs>(
        &mut self,
        other: &mut GlobalConsumer<T, Cs, Rs>,
        count: Option<usize>,
    ) -> usize
    where
        Cs: Container<T>,
...
impl<T: Copy, C, R> GlobalProducer<T, C, R>
where
    C: Container<T>,
    R: RingBufferRef<T, C>,
{
...
#[cfg(feature = "std")]
impl<C, R> GlobalProducer<u8, C, R>
where
    C: Container<u8>,
...
#[cfg(feature = "std")]
impl<C, R> Write for GlobalProducer<u8, C, R>
where
    C: Container<u8>,
    R: RingBufferRef<u8, C>,
{
